<?php
    include("./db_con.php");
    $ownSeminarQuery = "SELECT seminars.*, COUNT(seminar_participants.seminar_id) as participants_count FROM seminars LEFT JOIN seminar_participants ON seminars.seminar_id = seminar_participants.seminar_id LEFT JOIN org_list ON org_list.org_id = seminars.org_id WHERE ( seminars.isRemoved = 0 AND seminars.isFinished = 0 ) GROUP BY seminars.seminar_id";
    ...
    if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_assoc($result)){
            ...
            echo "<div class='btnclass'><a href='#?seminar_id=" . $row['seminar_id'] . "' id='button-30'>View</a></div>";
            echo "</div>";
        }
    }
    else{
